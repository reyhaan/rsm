function Sets = getCommunities(A,dp)
%GETCOMMUNITIES A is a graph in adjacent matrix, dp is the dependent
%parameter set by users. The return is a set of communities in nodes set
%form.
%   Detailed explanation goes here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

EM = (A <= dp); % Efficent paths matrix. Only these paths will be 
                % used in community construction. Other paths will
                % not be considered since such high resistance distance
                % is not allowed in a community.
EM = EM - diag(diag(EM));
Sets = maximalCliques(EM);
end
